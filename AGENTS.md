...
## Phase3-E 临床验证 MVP 建议评估（非已上线能力）

Phase3-E 的建议评估范围仅包括三项可核对的规则展示任务：当前五运六气、
任意年度分析，以及由临床人员主动输入北京标准时间后的指定时点分析。
Phase3-E1 在 Workbench 中实现精确的 /yunqi/calculate 入口，但该入口仍只
提供冻结规则的计算与展示；是否进入真实临床工作流必须等待后续专家验证和
独立 Gate，不得把它描述为问诊或医疗决策能力。

该建议评估不授权患者档案、HIS/EMR、问诊记录、症状采集、诊断、辨证、治疗、
处方、用药、风险预测、推荐、AI、持久化或新的医疗 Contract。真实临床事实、
专家参考答案、身份权限和责任边界必须先由相应专家与正式设计阶段确认，再进入
后续 Service、OpenAPI、@yunqi/contracts、@yunqi/client 和 React 链路。
