...
- 已测试年份范围：1900–2100；Phase 1 不声明 BCE 或五位数公历年份支持。
...
6. `src/index.ts` 保留 Tasks 1–5 已审查的入口，不删除或替换既有 API；增量导出解释模板，同时继续公开三个主服务、Provider 类型/默认适配器、公共结果类型和 `RULE_VERSION`。
...
| 2. 年干支 | `ganzhi/stem-branch.ts` | `year-and-suiyun.test.ts` 的 60-cycle、2044 回环、1900–2100 | 通过 |
...
2. 增加模板模块但尚未接入/导出后，同一命令仍 exit 1：10 项中 4 项失败，分别证明根入口缺少解释 helper、服务仍返回空集合、每次计算仍共享同一个空集合。
...
以下命令均在 `packages/yunqi-domain` 执行，记录的是 Task 6 代码与 README 完成后的 fresh 结果：
...
| `git diff --cached --check` | exit 0；10 个暂存文件无空白错误，范围仅为 Task 6 与明确的 Task 3 输入诊断收口 |
