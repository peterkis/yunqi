...
- 字符串输入只接受 `YYYY-MM-DDTHH:mm:ss`，其后可选 1–3 位毫秒（`.S`、`.SS` 或 `.SSS`），末尾必须带 `Z` 或 `±HH:mm` UTC 偏移，例如 `2024-05-20T21:00:00+08:00` 与 `2024-03-20T03:06:25.987Z`；无时区字符串会被拒绝。
- 不存在或溢出的公历日期时间字段（例如 `2024-02-30`、非闰年的 `02-29`、`04-31` 或 `24:00:00`）会抛出描述性 `RangeError`，不会被 JavaScript 日期解析器自动规范化。
- `Date` 输入按其绝对毫秒值解释；无效 `Date` 会被拒绝。
